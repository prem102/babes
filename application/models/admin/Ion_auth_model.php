...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 **/
	public function identity_check2($username = '')
	{
		$this->trigger_events('identity_check');
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array()){
		...
		$manual_activation = $additional_data['active'];
		if ($this->identity_check($identity))
		{
			$this->set_error('account_creation_duplicate_identity');
			setMessage('Email already exists','warning');
			return FALSE;
		}
		if ($this->identity_check2($additional_data['username']))
		{
			$this->set_error('account_creation_duplicate_identity');
	 ...
	 * @author Mathew
	 **/
	public function signup($identity, $password, $email, $additional_data = array(), $groups = array()){
		$this->trigger_events('pre_register');
		$manual_activation = $additional_data['active'];
		if ($this->identity_check($identity))
		{
	...
	**/
	...
	{ 
		...
		$query = $this->db->select($this->identity_column . ', email, u.id, password, active,last_login,first_name,last_name,company,phone,image')
		                  ...
		                  ->where($this->identity_column, $identity)
		                   ->where_in('ug.group_id', array(2,3))
		                    ->where('u.active', '1')
		                  ->limit(1)
		                  ->get('users as u');
		print $this->db->last_query();
