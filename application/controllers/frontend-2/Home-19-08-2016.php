...
class Home extends CI_Controller {
	 ...
	 */
	function index(){
		$data['js']=array('jquery-ui','jquery.selectBoxIt','jquery.flexslider'); 
		$data['meta']=array();
		$data['css']=array('flexslider','tab','common','home');
		try{
			$services		= $this->Home_model->getServices();
			if($services){
			...
			$featuredGirls	= $this->Home_model->allStaff($featured=1);
			if($featuredGirls){
				$data['featuredGirls'] = $featuredGirls;
			}
	...
	function search(){
		...
		try{
			...
			if(!empty($postData)){
				...
				$hairColors	= $this->Home_model->getHairColors();
				$data['hairColors'] = $hairColors;
				$eyeColors	= $this->Home_model->getEyeColors();
				$data['eyeColors'] = $eyeColors;
				$bodyTypes	= $this->Home_model->getBodyTypes();
				$data['bodyTypes'] = $bodyTypes;
				$bustTypes	= $this->Home_model->getBustTypes();
				$data['bustTypes'] = $bustTypes;
				$ethnicities= $this->Home_model->getEthnicities();
		...
		try{
			...
			if(!empty($staffs)){
				foreach($staffs as $staff){
					$name = !empty($staffs->first_name) ? $staff->first_name .' '.$staff->last_name : '' ;
					...
					$html .= '<div class="mask"><span class="form-rate">form $'.$minPrice.'</span></div></div></a>';
				}
			}
	...
	function staffs($username){
		try{
			...
			$data['js']		= array('jquery-ui','jquery.flexslider'); 
			$data['meta']	= array();
			$data['css']	= array('flexslider','common','staffdetails');
